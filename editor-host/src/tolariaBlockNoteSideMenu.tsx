// ---------------------------------------------------------------------------
// TolariaSideMenu — custom BlockNote SideMenu (worklist 9.3.1)
// ---------------------------------------------------------------------------
//
// Ported verbatim from `src/components/tolariaBlockNoteSideMenu.tsx`
// (the React-era custom side menu).  The default BlockNote SideMenu
// gives us a `+` add-block button + a `⋮⋮` drag handle, but:
//
//   - The drag handle's HTML-5 `dragstart` flow is jittery in the
//     embedded WKWebView (the native shell intercepts the OS drag
//     gesture); the React app worked around this with a pointer-based
//     reorder that runs entirely inside ProseMirror's transactional
//     API.
//   - The default add-block button always inserts a fresh paragraph
//     and pops the slash menu — but for *empty* blocks we instead
//     pin the cursor on the existing block and open the slash menu
//     in-place (avoids stranding an empty paragraph between blocks).
//   - The default drag-handle menu shows BlockNote's "Colors" pane,
//     which is meaningless inside a plaintext markdown round-trip.
//
// The CSS hook `.tolaria-block-drag-handle` (consumed by
// `editor-host/src/style.css`) is added by the wrapping `<span>`
// around BlockNote's `SideMenu.Button` — that's the load-bearing
// class for the styling parity row.
//
// Icons: the React app ships @phosphor-icons/react; the editor-host
// bundle is single-file and adding the icon package would balloon
// the bundle by ~100 kB.  We inline two SVGs instead — they match
// the visual weight of `Plus` (20 px line glyph) and
// `DotsSixVertical` (the BlockNote-style grip column) closely
// enough that the user can't tell them apart in QA.
...
function runSideMenuAction(action: () => void) {
    try {
        ...
        console.warn("[editor-host] Ignored stale block side-menu action:", error);
    }
}
...
function editorBlockElement(editor: TolariaBlockNoteEditor): HTMLElement | null {
    ...
    return element.matches(".bn-editor")
        ? element
        : (element.querySelector(".bn-editor") as HTMLElement | null);
}
...
function sideMenuElementForEditor(editorElement: HTMLElement): HTMLElement | null {
    const container = editorElement.closest(".editor-host-container") ?? editorElement;
...
function blockTextAnchorRect(blockElement: HTMLElement): DOMRect | null {
    ...
    const firstLineRect = Array.from(range.getClientRects()).find(
...
function createSideMenuResizeObserver(onResize: () => void): ResizeObserver | null {
    return typeof ResizeObserver === "undefined" ? null : new ResizeObserver(onResize);
}
...
function createSideMenuAlignmentController(editor: TolariaBlockNoteEditor, blockId: string) {
    ...
    const observeTargets = () =>
        observeSideMenuAlignmentTargets({
            blockId,
            editorElement,
            resizeObserver,
            state,
...
function useSideMenuTextAlignment(
...
function styleDragPreview(preview: HTMLElement, rect: DOMRect) {
    ...
    preview.className = "editor-host-container";
...
function createDropIndicator(ownerDocument: Document): HTMLElement {
    ...
    indicator.style.boxShadow =
        "0 0 0 1px rgba(21, 93, 255, 0.12), 0 0 10px rgba(21, 93, 255, 0.28)";
...
function updateDropIndicator(
    affordances: ReorderAffordances | undefined,
    ...
    affordances.dropIndicator.style.top = `${
        target.placement === "before" ? rect.top - 1 : rect.bottom - 1
    }px`;
...
function useSideMenuBlock() {
    ...
    const block = useExtensionState(SideMenuExtension, {
        editor,
        selector: (state): SideMenuBlock | undefined =>
            state?.block
                ? {
...
// Inline SVGs — visually equivalent to phosphor's `Plus` and
// `DotsSixVertical` at 20 px, but zero added bundle weight.
function PlusIcon() {
    return (
        <svg
        ...
        (event: ReactPointerEvent<HTMLElement>) => {
            ...
            runSideMenuAction(() => {
                ...
                const handlePointerUp = (nativeEvent: PointerEvent) =>
                    finishPointerReorder(nativeEvent);
...
function TolariaTableHeaderItem({
    ...
    const checked =
        header === "row" ? Boolean(tableContent.headerRows) : Boolean(tableContent.headerCols);
            ...
            onClick={() => {
                runSideMenuAction(() => {
                    ...
                    editor.updateBlock(currentBlock.id, {
                        content: {
                            ...currentContent,
                            [header === "row" ? "headerRows" : "headerCols"]: checked
                                ? undefined
                                : 1,
                        } as never,
                    });
...
function TolariaDragHandleMenu() {
    ...
    return (
        <DragHandleMenu>
            ...
            <TolariaTableHeaderItem header="row">
                {dict.drag_handle.header_row_menuitem}
            </TolariaTableHeaderItem>
            <TolariaTableHeaderItem header="column">
...
/**
 * Custom BlockNote SideMenu — wraps the default `<SideMenu>` with our
 * `+` add-block + `⋮⋮` drag-handle controls.  Mount it via
 * `<SideMenuController sideMenu={TolariaSideMenu} />` (see
 * `menus.tsx`).
 *
 * The CSS hook `.tolaria-block-drag-handle` on the trigger `<span>`
 * is what the editor-host stylesheet keys off for the parity with
 * the React app.
 */
