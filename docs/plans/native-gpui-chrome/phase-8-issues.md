...
> **Phase 8 status: CLOSED (2026-05-21).**  All Blockers, all section-2 bug rows, and both section-3 chrome rows resolved on `feat/native-gpui-chrome`.  Six note-toolbar product features (2.9–2.14, 2.17) deferred to Phase 9 — they are not regressions but new feature work, scoped separately from chrome stabilisation.  See **Close-out** section at end of file for the full ledger.
...
2.9. ➡️ note-toolbar-star element is not wired — deferred to Phase 9 (heavy product feature, see close-out)
2.10. ➡️ note-toolbar-organized element is not wired — deferred to Phase 9
2.11. ➡️ note-toolbar-neighborhood element is not wired — deferred to Phase 9
2.12. ➡️ note-toolbar-raw element is not wired — deferred to Phase 9
2.13. ➡️ note-toolbar-ai element is not wired — deferred to Phase 9
2.14. ➡️ note-toolbar-toc element is not wired — deferred to Phase 9
2.15. ✅ note-toolbar-reveal element is not wired
2.16. ✅ note-toolbar-copy-path element is not wired
2.17. ➡️ note-toolbar-more element is not wired — deferred to Phase 9
...
| 2. High Priority — new product features | 0 / 6 | ➡️ Phase 9 (2.9, 2.10, 2.11, 2.12, 2.13, 2.14, 2.17) |
...
|-----|--------------|-------|
| 2.9 | `note-toolbar-star` | Wire to user-vault "starred notes" persistence (no current store). |
| 2.10 | `note-toolbar-organized` | Auto-organise into folder by type/date — needs vault-policy primitives. |
| 2.11 | `note-toolbar-neighborhood` | Backlinks/forward-links graph view — non-trivial UI surface. |
| 2.12 | `note-toolbar-raw` | Toggle raw-Markdown view inside the WKWebView — coordinates with `editor-host`. |
| 2.13 | `note-toolbar-ai` | AI assistant entry point — requires LLM provider plumbing. |
| 2.14 | `note-toolbar-toc` | Outline panel — render-only; reuses `editor-host` outline emission. |
| 2.17 | `note-toolbar-more` | Overflow menu hosting items 2.9-2.14 plus future actions. |

Each row in section 2 of this doc keeps its `➡️` marker so the Phase 9 plan can copy them across without re-discovery.
...
- `2c680c9a` feat(tolaria): minimal GPUI inspector renderer — fixes invisible toggle (3.1 follow-up)
- `2f3c7101` docs(phase-8): close worklist 3.1 — GPUI inspector overlay restored to original 5cd51756 semantic
- `62d64673` fix(tolaria,actions): restore GPUI inspector overlay as ToggleInspector — revert 6c14e83c misinterpretation (3.1)
- `a0971295` docs(phase-8): close worklist row 2.31 — Angle-C2 all three phases landed
- `649a686c` refactor(chrome): replace OverlayTooltipExt fan-out with inline gpui_component::Tooltip — Angle-C2 Phase 3 (2.31)
- `e9214f51` fix(inspector_panel): paint root with theme.background + theme.foreground (3.1 — superseded by 62d64673; theme tokens still apply if InspectorPanel is reused)
- `b4dd5efe` fix(note_item,ui): WKWebView send-to-back z-order + UI objc2 deps — Angle-C2 Phase 2 (2.31)
- `f0a38d84` feat(tolaria,chrome): transparent workspace base layer — Phase 1 of Angle C2 (2.31)
- `b227ea53` fix(editor-host): byte-identical round-trip + remove PropertiesPanel (2.26 + 2.27)
- `211717f9` perf+fix(ui): cache OverlayTooltip NSPanel + prefer-Above placement (2.28 + 2.30)
- `cfdde37e` test(editor-host): round-trip the user-named demo notes (2.27 follow-up)
- `6b19ddf5` fix(editor-host): auto-save was stripping YAML frontmatter from disk (2.27 regression)
- `caf6641e` feat(tolaria,workspace): dynamic Show/Hide menu labels for Sidebar & Inspector (3.2)
- `6c14e83c` feat(tolaria,actions): open InspectorPanel as a separate macOS window (3.1 — reverted by 62d64673)
