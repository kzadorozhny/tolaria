...
8.1.1. ✅ Clicking on the notes search crashes with paniic
8.1.2. ✅ Note web view renders and goes blank once the mouse moved over it

## 2. High Priority

8.2.1. ✅ Note list top bar title need to reflect the title of the iems selected in the side bar
8.2.2. ✅ On startup Note List shows some note is slected. The note view should show an empty note state. It is the same as in React variant.
8.2.3. ✅ Active Projects view filer does not work
8.2.4. ✅ All clikable buttons lack the hints
8.2.5. ✅ Vault picker popup does not close on focs loss
8.2.6. ✅ Side bar Types, Views, Folder are not collpsable
8.2.7. ✅ System menu is missing items for File, View, Help
8.2.8. ✅ Note web view lacks any styling present in React variant
8.2.9. ➡️ note-toolbar-star element is not wired
8.2.10. ➡️ note-toolbar-organized element is not wired
8.2.11. ➡️ note-toolbar-neighborhood element is not wired
8.2.12. ➡️ note-toolbar-raw element is not wired
8.2.13. ➡️ note-toolbar-ai element is not wired
8.2.14. ➡️ note-toolbar-toc element is not wired
8.2.15. ✅ note-toolbar-reveal element is not wired
8.2.16. ✅ note-toolbar-copy-path element is not wired
8.2.17. ➡️ note-toolbar-more element is not wired
8.2.18. ✅ note-toolbar-inspector element is not wired
8.2.19. ✅ Notes list top bar Add bottom does nothing
8.2.20. ✅ Notes list sort dropdown does not update the title after the user selects an option
8.2.21. ✅ Notes list sort dropdown appears under the web view pannel
8.2.22. ✅ Notes list top bar search: Esc button shodu close the search line and clear the search query
8.2.23. ✅ The sidebar-types-sort button should be Types Filter button
8.2.24. ✅ Install @blocknote/shadcn and restore BlockNote menu UI primitives
8.2.25. ✅ Redirect WebView console logs to the tolaria in-process simple logger
8.2.26. ✅ Round-tripping open/save reformats frontmatter
8.2.27. ✅ Frontmatter is rendered in preview mode
8.2.28. ✅ Match the note tollbar tolltips styles to the rest of the UI
8.2.29. ✅ Properties panel — add/remove/edit controls (type-aware editors for date/boolean/wikilink/list)
8.2.30. ✅ Notes list sort dropdown tooltip is out of place
8.2.31. ✅ Inline chrome overlays via transparent GPUI base layer (Angle C2)

## 3. Low Priority

8.3.1. ✅ Inspector view should be opened in a separate windows, not a pannel
8.3.2. ✅ System window menu should display Show Sidebar|Hide Sidebar, Show Inspector|Hide Inspector depending on the current state
...
3. **`ToggleInspector` action handler.**  Rebuilds via the active-window-lookup `rebuild_menus(cx)` helper immediately after the slot mutation completes (open or close).  Covers menu clicks, `Cmd`-keyed accelerators, and the note-toolbar inspector button from worklist 8.2.18 — every entry point already dispatches through `actions::ToggleInspector`.
